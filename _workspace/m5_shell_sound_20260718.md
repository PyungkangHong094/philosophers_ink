...
## 0. P1 핫픽스 — 게임 시작 시 "지잉" 소음 (2026-07-18)

**증상**: 레벨 플레이 시작 시 거슬리는 연속 "지잉" 버즈.

**원인 2가지**:
1. (주범) **앰비언트 그레인** — `WaveForm.saw`, superWave, 디튠 0.4를 `looping:true`로 재생하는
   연속 루프. 볼륨을 활성 셀 밀도로 변조하는데, 레벨 001은 방출구가 즉시 입자를 쏟아 밀도가
   바로 차오른다 → 디튠 슈퍼웨이브 톱니 루프가 끊임없이 울리는 드론. 밀도 기반 "연속 루프"는
   구조적으로 드론이 될 수밖에 없다(움직임이 아니라 존재량에 반응).
2. (부차) **착수 틱 무스로틀** — `flaskFill`이 셀 착수마다 발화. 입자가 초당 수십 셀 착수 →
   삼각파 블립이 겹쳐 버즈.

**수정**:
- 앰비언트 그레인 **기본 OFF** (`SfxSpec.ambientGrainEnabled = false`). 그레인 소스를 아예
  로드하지 않고 `setAmbientDensity`·PlayScreen 밀도 스캔을 게이트로 차단. GDD 9.2의 진짜 파티클
  그레인은 낙하·퇴적 "움직임" 이벤트가 필요한데, sim에 그 훅이 생기면(현재 phase-change 콜백은
  등장) 이벤트 기반으로 재설계해 다시 켠다. 그 전까지는 드론을 켜지 않는다("없는 게 낫다").
- `flaskFill` **스로틀 70ms**(≈14틱/s 상한) — 카운트업 만족감은 유지하되 겹침 버즈 제거.

**추가 증상 — 화면 이탈/백그라운드 후에도 소음 지속 (오디오 생명주기)**:
- 루프성 핸들이 화면 dispose에서 정지되지 않으면 전역 SoLoud에 남아 계속 울린다(생명주기 버그).
- 수정:
  - `AudioService.stopAll()` 도입 — 앰비언트·향후 BGM 등 **루프성 재생 전부 정지**(원샷은 자연 종료).
  - `PlayScreen.dispose()`에서 `stopAll()` 호출(기존 stopAmbient 대체) — 화면 이탈 시 루프 전역 잔존 제거.
  - `WidgetsBindingObserver` 등록 → `didChangeAppLifecycleState`가 resumed 외 상태(백그라운드/비활성)
    에서 `stopAll()` + 시뮬 일시정지. 프레임 정지 핫패스도 `stopAmbient()` 유지.
  - 회귀 테스트 `test/ui/play_lifecycle_test.dart`(2케이스): 재생 상태 추적 `_RecordingAudio`로
    (a) 화면 이탈(dispose) 시 stopAll 호출·루프 정지, (b) 앱 백그라운드 전환 시 stopAll 호출·루프 정지.

**검증**:
- 구조적 증명: 코드베이스의 유일한 `looping:true` 재생이 이제 `ambientGrainEnabled`(false) 게이트
  뒤에 있고 `_grain` 소스 자체가 로드되지 않는다 → **연속 음원이 생성/재생될 수 없다.** 설령 향후
  루프를 켜더라도 화면 dispose·백그라운드에서 `stopAll()`이 정지를 보장한다(생명주기 테스트로 고정).
  남는 오디오는 전부 ≤220ms 원샷 블립(페이드아웃+정지 예약)이며 stroke 45ms·flask 70ms 스로틀.
  ⇒ 지속 버즈가 구조적으로 불가능.
- 회귀 가드 테스트: `SfxSpec.ambientGrainEnabled == false` + 스로틀 존재 단언(`test/audio`) +
  생명주기 2케이스(`test/ui/play_lifecycle_test`).
- `dart analyze`(내 소유) 클린 + 전체 `flutter test` **218개 통과**.
- iOS 시뮬레이터 디버그 빌드 재성공(오디오 경로 변경 어셈블 검증).
- ⚠️ **청감 확인의 한계**: 이 환경에선 오디오를 들을 수 없다. 위 구조적 증명(연속 음원 제거)과
  테스트로 대체하며, 최종 귀 확인은 사람(또는 부착된 flutter run 세션)의 몫이다. 확인 방법:
  시뮬레이터에서 L001 시작 → 그리기 → 클리어. 이제 들리는 소리는 (a) UI/획/착수 원샷 틱과
  (b) 클리어 스팅어뿐, 배경 드론 없음.
